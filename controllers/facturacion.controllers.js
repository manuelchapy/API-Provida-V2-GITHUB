...
facturacionCtrl.crearFacturaOrdenTrabajo = async(req, res) =>{
    ...
    if(req.body.id_tipo_factura == 1 || req.body.id_tipo_factura == 2){
        ...
        if(vueltosPesosToDolares > 0  || vueltosBolivaresToDolares > 0 || sumaDolaresVueltos > 0){
            dolaresRedondeado = (Math.round((dolaresTotales - vueltosDolaresTotales - igtfDolarTotal)+ "e+2") + "e-2");
            bolivaresRedondeado = dolaresRedondeado * req.body.tasa_bolivar_dia
        }else{
            dolaresRedondeado = (Math.round(dolaresTotales+ "e+2") + "e-2");
            bolivaresRedondeado = dolaresRedondeado * req.body.tasa_bolivar_dia;
        }
        
        
        req.body.bolivaresRedondeado = (Math.round(bolivaresRedondeado+ "e+2") + "e-2");
        req.body.dolaresRedondeado = (Math.round(dolaresRedondeado+ "e+2") + "e-2");
        ...
        async function crearFactura(numFact){
            ...
            if(req.body.id_tipo_factura == 1){
                ...
                for(const itemPago of req.body.pagos){
                    ...
                    if(nombreDivisa == "DOLARES"){
                        sumaDolares = sumaDolares + itemPago.monto;
                    }else if(nombreDivisa == "PESOS"){
                        sumaPesos = sumaPesos + itemPago.monto;
                    }else if(nombreDivisa == "BOLIVARES"){
    ...
    function detallesDeFactura(factura){
        ...
        const sql = 'INSERT INTO tbl_detalle_factura_paciente (id_factura, id_registro_convenio, id_paciente, id_examen, id_cultivo) VALUES?';
    ...
    function detallesFiscales(factura){
        ...
        if(myValues == 0){
            ...
            const sql = 'INSERT INTO `tbl_registro_pago` (id_factura, id_registro_divisa, id_tipo_pago, id_banco, numero_referencia, tipo_registro, monto, igtf_pago, fecha_creacion, igtf_monto) VALUES?';
            connection.query(sql, [myValues], (err, result) => {
                if (err) {
                    console.log('no se pudo a agregar', err)
                    res.send('ERROR!')
...
facturacionCtrl.modificarPago = async(req, res) =>{
    ...
    if(agregarModificar.length > 0){
        for(const item of agregarModificar){
            ...
            if(item.accion == 1){
                //console.log("LLEGO 1")
                await crearRegistroPago(item, idFactura);
            }
            if(item.accion == 2){
                //console.log("LLEGO 2")
    ...
    async function modificarRegistroPago(registroPago){
        return new Promise((resolve, reject) => {
            ...
            if(registroPago.id_banco == null || registroPago.id_banco == 'null'){
                const sql = "UPDATE tbl_registro_pago SET id_registro_divisa = '" + registroPago.id_registro_divisa + "', id_tipo_pago = '" + registroPago.id_tipo_pago + "', id_banco = NULL, numero_referencia = NULL, monto = '" + registroPago.monto + "' WHERE id_registro_pago = '" +registroPago.id_registro_pago+ "'";
                ...
                const sql = "UPDATE tbl_registro_pago SET id_registro_divisa = '" + registroPago.id_registro_divisa + "', id_tipo_pago = '" + registroPago.id_tipo_pago + "', id_banco = '" + registroPago.id_banco + "', numero_referencia = '" + registroPago.numero_referencia + "', monto = '" + registroPago.monto + "' WHERE id_registro_pago = '" +registroPago.id_registro_pago+ "'";
                connection.query(sql, function (error, result, fields) {
                    if (result) {
                        resolve('1');
                    }else if(error){
    ...
    async function crearRegistroPago(registroPago, idFactura){
...
facturacionCtrl.crearFacturaOrdenTrabajoConvenios = async(req, res) =>{
    ...
    function extraerFactura(num){
        ...
        function detallesDeFactura(factura){
            //console.log('EN DETALLES DE FACTURA CONVENIO', req.body)
...
facturacionCtrl.imprimirFactura = async(req, res) =>{
    const sql = "SELECT tbl_detalle_factura_paciente.id_detalle_factura_paciente, tbl_detalle_factura_paciente.id_paciente, tbl_detalle_factura_paciente.id_examen, tbl_detalle_factura_paciente.id_cultivo, tbl_detalle_orden.id_orden, tbl_orden.numero_orden FROM `tbl_detalle_factura_paciente`  INNER JOIN `tbl_detalle_orden` ON tbl_detalle_orden.id_detalle_factura_paciente = tbl_detalle_factura_paciente.id_detalle_factura_paciente INNER JOIN tbl_orden ON tbl_detalle_orden.id_orden = tbl_orden.id_orden WHERE tbl_detalle_factura_paciente.id_factura='" +req.params.id_factura+"'";
    connection.query(sql, function (err, result, fields) {
        if (err) {
            console.log('ERROR en CheckTemplate', err);
            res.send('3');
